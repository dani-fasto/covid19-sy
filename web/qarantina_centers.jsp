<%-- 
    Document   : qarantina_centers
    Created on : Jun 7, 2020, 3:36:16 AM
    Author     : ZORO
--%>
...
<html lang="en">
  ...
  <body onload="myMap()">
      ...
      <sql:query var="map_qarantina" dataSource="${db}" > select * from qarantina_centers_t </sql:query>
    ...
    <nav class="navbar navbar-expand-md navbar-dark bg-dark sticky-top">
      ...
      <div class="landing-text text-center">
        <h1 style="padding: 2.5%;">مراكز الحجر الصحي و توزعها في سورية</h1>
      </div>
      <br>
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    ...
    <!--javascript files and functions-->
        ...
        var markers = [
            <c:forEach var ="qarantina" items ="${map_qarantina.rows}" >
                    [  '<c:out value="${qarantina.id}" />',
                       '<c:out value="${qarantina.qarantina_name}" />',
                       '<c:out value="${qarantina.qarantina_address}" />',
                       '<c:out value="${qarantina.qarantina_tele}" />',
                       '<c:out value="${qarantina.qarantina_lat}" />',
                       '<c:out value="${qarantina.qarantina_long}" />'  ],
            </c:forEach>];

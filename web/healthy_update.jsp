...
<html lang="en">
  ...
  <body>
    ...
    if(request.getParameter("submit")!=null)
    {
     ...
     pst=con.prepareStatement("update healthy_centers_t set healthy_name=?, healthy_address=?, healthy_tele=? ,healthy_lat=? ,healthy_long=? where id=? ");
     ...
     <script>
       alert("تمت تحديث بيانات المركز بنجاح ! ");
                    window.location.href = "healthy_centers_t.jsp";
      </script>
        ...
        <div class="col-sm-4">
            ...
            <form method="POST" action="#" id="add_healthy" name="add_healthy">
                        <% 
                            ...
                            con=DriverManager.getConnection("jdbc:mysql://localhost:3306/covid19?useUnicode=true&characterEncoding=utf8", "root", "root");
                            String id= request.getParameter("id");
                            pst=con.prepareStatement("select * from healthy_centers_t where id=?");
                            pst.setString(1, id);
                            rs=pst.executeQuery();
                            ...
                            while (rs.next())
                             {
                                ...
                                <input type="text" value="<%=rs.getString("healthy_name") %>" name="healthy_name" class="form-control" id="healthy_name" placeholder="اسم المركز" required style="margin-bottom: 40px;" >
                                </div>
                                <div class="form-group">
                                <label> عنوان المركز</label>
                                <input type="text" value="<%=rs.getString("healthy_address") %>" name="healthy_address" class="form-control" id="healthy_name" placeholder="عنوان المركز" size="20px" required  style="margin-bottom: 40px;">
                                </div>
                                <div class="form-group">
                                <label> هاتف المركز</label>
                                <input type="text" value="<%=rs.getString("healthy_tele") %>" name="healthy_tele" class="form-control" id="healthy_name" placeholder="هاتف المركز" size="20px" required  style="margin-bottom: 40px;">
                                </div>
                                <div class="form-group">
                                <label> (احداثيات المركز (خط الطول </label>
                                <input type="text" value="<%=rs.getString("healthy_lat") %>" name="healthy_lat" class="form-control" id="healthy_name" placeholder="احدائيات المركز" size="20px" required style="margin-bottom: 40px;" >
                                </div>
                                <div class="form-group">
                                <label> (احداثيات المركز (خط العرض</label>
                                <input type="text" value="<%=rs.getString("healthy_long") %>" name="healthy_long" class="form-control" id="healthy_name" placeholder="احدائيات المركز" size="20px" required style="margin-bottom: 40px;" >
                                </div>
